  ...
  try {
    ...
    if (!flag) {
      ...
      if(!buttonType){
        ...
        promptText += ` 대답은'년 월 일 (요일) 점심: 밥(200kcal, 1g 지방, 4g 단백질, 40g 탄수화물) + 닭가슴살 볶음(200kcal, 8g 지방, 16g 단백질, 4g 탄수화물)'와 같이 ${dateString}을 기준으로 7일치 식단을 구성해주세요. 간식은 제외해주세요.`;
        ...
        const { data } = req.body;
        const promptText = `${data} 이 문장에서 식단 스케줄을 MySQL에 입력하려 합니다. Json형태로 가공해주세요. [{key:value}]형태로 대답해주세요. 키값은 date타입인 날짜(createAt) 아침이면 08:00:00 점심이면 12:00:00 저녁이면 19:00:00추가, 음식명(name), 칼로리(calorie), 단백질(protein), 지방(fat), 탄수화물(cho)로 나뉘게 해주세요. 셋트수가 1이상이면 횟수(count)에 곱해주세요.`;
        const response = await openai.createChatCompletion({
          model: "gpt-3.5-turbo",
          messages: [
      ...
      if (!buttonType) {
        ...
        const { data } = req.body;
        const promptText = `${data} 이 문장에서 운동 스케줄을 MySQL에 입력하려 합니다. Json형태로 가공해주세요. [{key:value}]형태로 대답해주세요. 키값은 date타입인 날짜(createAt), string타입인 운동명(type), INTERGER타입인횟수(count)로 나뉘게 해주세요. 셋트수가 1이상이면 횟수(count)에 곱해주세요.`;
        const response = await openai.createChatCompletion({
          model: "gpt-3.5-turbo",
          messages: [
            // {"role": "system", "content":"you are a Database Manager"},
